...
The use of maildirs-extension can be enabled by setting the variable
=mu4e-use-maildirs-extension= to a non-nil value:
...
The maildirs extension is not enabled by default. To activate it, us the
variable =mu4e-use-maildirs-extension= as described in the install section.
